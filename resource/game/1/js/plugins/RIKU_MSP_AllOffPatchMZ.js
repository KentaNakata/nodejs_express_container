...
/*:ja
 * @target MZ
 * @plugindesc RIKU_MultiSoundPlayerで設定したサウンドを終了時などに一括で解除します。
 * @author 沫那環(Tamaki Awana)
 *
 * @base RIKU_MultiSoundPlayer
 *
 * @help n2naokun(柊菜緒)さんMV版作、リクドウさん移植の、
 * RPGツクールMZプラグイン素材「RIKU_MultiSoundPlayer」に、
 * タイトル画面移行時やセーブデータのロード時の暗転に
 * 設定していたサウンドを一括解除する機能を追加します。
 * RIKU_MultiSoundPlayer直下にこのプラグインを導入するだけで使えます。
 * ※このプラグイン単体では使えません！
 *
 *
 * 【プラグインコマンドについて】
